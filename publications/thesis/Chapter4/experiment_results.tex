...
Bảng~\ref{tab:autoshotv2_results} tổng hợp F1-score của toàn bộ các phiên bản đã thực hiện trong khóa luận cùng các mô hình tham chiếu (TransNetV2, AutoShot gốc và một số công trình kinh điển) trên ba bộ dữ liệu SHOT, BBC và ClipShots, nhằm kiểm chứng các giả thuyết nghiên cứu RQ1 và RQ3. Nhóm AutoShotV2 gồm bốn biến thể khác nhau ở \textit{tập dữ liệu dùng huấn luyện lại tầng phân loại} (Pha 2): SHOT+ClipShots (cấu hình triển khai chính, ngưỡng cố định $\theta=0.10$), chỉ SHOT, chỉ ClipShots, và chỉ ClipShots kèm trung bình trượt lũy thừa trọng số (EMA). Ô ``--'' là cấu hình chưa được đánh giá trên tập tương ứng.
...
\caption[So sánh F1-score các phiên bản trên SHOT, BBC và ClipShots]{So sánh F1-score của toàn bộ các phiên bản và mô hình tham chiếu trên SHOT, BBC và ClipShots. AutoShotV2 (Clip+Shot) là cấu hình triển khai ngưỡng cố định ($\theta=0.10$); AutoShotV2-HeatMap báo cáo ở ngưỡng cho F1 cao nhất trên mỗi tập. Ô ``--'' là cấu hình chưa được đánh giá trên tập tương ứng.}
...
AutoShot (paper) & AutoShot gốc, số công bố & 0.8410 & 0.9710 & 0.7870 \\
...
AutoShotV2 (Clip+Shot) & Train lại head trên SHOT+ClipShots (triển khai chính) & 0.8545 & 0.9656 & 0.7529 \\
AutoShotV2 (only Shot) & Train lại head chỉ trên SHOT & 0.8552 & 0.9512 & 0.7241 \\
AutoShotV2 (only ClipShots) & Train lại head chỉ trên ClipShots & 0.8371 & 0.9494 & 0.7638 \\
AutoShotV2 (only ClipShots + EMA) & Như trên, thêm trung bình trượt EMA trọng số & 0.8417 & 0.9604 & 0.7860 \\
AutoShotV2-HeatMap & + nhãn mềm dạng heatmap (thiên Recall) & 0.8557 & -- & 0.7560 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Nhận xét:} Bảng~\ref{tab:autoshotv2_results} cho thấy các kiến trúc thế hệ sau (từ TransNetV2 trở đi) vượt trội rõ rệt so với các công trình kinh điển (DSM, TransNet V1). Trong nhóm AutoShotV2, \textbf{không phiên bản nào thắng tuyệt đối trên cả ba miền}: trên SHOT các biến thể có dùng SHOT khi huấn luyện đạt cao nhất (HeatMap $0.8557$, only-Shot $0.8552$, Clip+Shot $0.8545$), đều vượt AutoShot gốc ($0.8405$); trên ClipShots biến thể huấn luyện trên ClipShots kèm EMA đạt cao nhất ($0.7860$) trong khi biến thể chỉ dùng SHOT tụt còn $0.7241$ --- khẳng định ClipShots nhạy với dữ liệu và cấu hình theo miền; trên BBC khoảng cách giữa các phiên bản nhỏ ($0.949$--$0.966$). Cấu hình \textbf{Clip+Shot} được chọn làm giải pháp triển khai chính vì cân bằng tốt nhất cả ba miền dưới \textit{một} ngưỡng chung, thay vì tối ưu cực đại riêng từng tập (mức trần lạc quan oracle được bàn ở Mục~\ref{sec:extra_param_sweep}).
