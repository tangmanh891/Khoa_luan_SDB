...
AutoShotV2-HeatMap & + nhãn mềm dạng heatmap (thiên độ bao phủ) & 0.8557 & -- & 0.7560 \\
\bottomrule
\end{tabular}
\end{table}
...
AutoShotV1 giữ nguyên toàn bộ trọng số AutoShot@F1 và \textbf{không huấn luyện lại tham số nào}, chỉ bổ sung làm mượt Gaussian 1D ($\sigma = 2.0$, dò trên tập xác thực SHOT) lên chuỗi xác suất trước bước ngưỡng hóa (Mục~\ref{subsec:v2_gaussian}). Chỉ với bước hậu xử lý này, F1 trên SHOT tăng từ $0.8405$ (AutoShot tự chạy lại) lên $0.8477$ ($+0.72$ điểm phần trăm), với độ chính xác dương $0.8558$ và độ bao phủ $0.8398$ --- một điểm vận hành thiên về độ chính xác dương. Kết quả xác nhận làm mượt Gaussian là thành phần nền tảng có ích; AutoShotV2 đầy đủ bổ sung thêm hàm mất mát Focal, nhãn nhiều điểm dương và hiệu chỉnh nhiệt độ chủ yếu để nâng độ bao phủ và cân bằng lại độ chính xác dương và độ bao phủ. Lưu ý AutoShotV1 khác cấu hình đối chứng $P1$ ở Mục~\ref{sec:ablation}: $P1$ có huấn luyện lại tầng phân loại bằng BCE, còn AutoShotV1 thì không.

\subsection{Biến thể AutoShotV2-HeatMap --- nhãn mềm dạng heatmap}
\label{sec:autoshotv2_heatmap}

AutoShotV2-HeatMap giữ nguyên mạng xương sống, chỉ đổi phía nhãn khi huấn luyện lại tầng phân loại: thay nhãn một điểm dương bằng nhãn mềm phân phối Gaussian ($\sigma_{\text{nhãn}}=3.0$) quanh ranh giới, kèm làm trơn nhãn bất đối xứng ($\varepsilon=0.1$, chỉ áp dụng cho nhãn dương), nhằm tăng khả năng bắt chuyển cảnh dần. Trên SHOT tại ngưỡng $\theta=0.10$, biến thể nâng độ bao phủ từ $0.8537$ (đầu phân loại cơ sở, không dùng nhãn dạng bản đồ nhiệt) lên $0.8804$ ($+2.67$ điểm phần trăm) trong khi F1 giữ tương đương ($0.8557$), đổi lại độ chính xác dương giảm còn $0.8324$. Trên ClipShots đạt F1 $0.7560$ (ngưỡng $0.15$). Như vậy HeatMap là một \textit{lựa chọn vận hành thiên về độ bao phủ}, phù hợp các kịch bản ưu tiên không bỏ sót ranh giới, chứ không nâng trần F1; BBC chưa được đánh giá do thiếu video gốc ở môi trường cục bộ.

\section{Kết quả chính: độ chính xác dương, độ bao phủ và F1 theo từng bộ dữ liệu}
\label{sec:extra_param_sweep}

\textbf{Mục đích thí nghiệm:} Mục này báo cáo kết quả chính của AutoShotV2 --- độ chính xác dương, độ bao phủ và F1 tại \textbf{ngưỡng cho F1 cao nhất trên từng bộ dữ liệu}, đúng theo giao thức đánh giá của AutoShot gốc\footnote{Tương ứng hàm đánh giá \texttt{mAP\_f1\_p\_fix\_r} trong mã nguồn AutoShot gốc: quét một dải ngưỡng quyết định rồi lấy F1 cao nhất cho mỗi bộ dữ liệu.}. Nhờ dùng chung giao thức, con số của AutoShotV2 và AutoShot được so sánh trực tiếp và công bằng.
...
\toprule
\textbf{Bộ dữ liệu} & \textbf{Ngưỡng} & \textbf{Precision} & \textbf{Recall} & \textbf{F1} & \textbf{$\Delta$F1 vs AutoShot} \\
\midrule
SHOT & 0.12 & 0.8408 & 0.8816 & \textbf{0.8607} & $+0.0202$ \\
ClipShots & 0.19 & 0.7011 & 0.8554 & \textbf{0.7706} & $+0.0057$ \\
BBC & 0.12 & 0.9668 & 0.9628 & \textbf{0.9648} & $+0.0094$ \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Thảo luận:} Trên SHOT --- bộ dữ liệu trọng tâm của đề tài --- AutoShotV2 đạt F1 $0.8607$, cao hơn AutoShot tự chạy lại $+2.0$ điểm phần trăm dưới cùng một giao thức chọn ngưỡng. Trên BBC, mô hình tổng quát hóa tốt ($0.9648$, cao hơn AutoShot $0.9554$ và TransNetV2 tự chạy lại $0.9593$). Trên ClipShots, F1 đạt $0.7706$ với độ bao phủ cao ($0.8554$) nhưng độ chính xác dương thấp hơn hẳn ($0.7011$): mô hình bắt được phần lớn ranh giới thật nhưng vẫn sinh nhiều dự đoán dư thừa trên phân phối video thế giới mở đa dạng --- đây là hạn chế chính còn tồn tại trên miền dữ liệu này. Lưu ý ngưỡng $0.12$ và $0.19$ được lấy trên lưới mịn hơn dải ngưỡng mặc định của AutoShot, và được áp dụng đồng nhất cho cả AutoShotV2 lẫn các mô hình tham chiếu để bảo đảm so sánh công bằng.

\section{Thực nghiệm phân rã thành phần}
\label{sec:ablation}

\textbf{Mục đích thí nghiệm:} Thực nghiệm phân rã thành phần này nhằm kiểm chứng độc lập và làm rõ đóng góp thực tế của từng đề xuất cải tiến, gồm hàm mất mát Focal và nhãn nhiều điểm dương ở giai đoạn huấn luyện, cùng hiệu chỉnh nhiệt độ và làm mượt Gaussian ở giai đoạn hậu xử lý. Thực nghiệm được tiến hành trên quy trình Pha 2 với mạng xương sống được đóng băng để đánh giá hiệu năng cuối cùng của hệ thống và trả lời câu hỏi nghiên cứu RQ2.

Bảng~\ref{tab:ablation_controlled} phân tách đóng góp của từng thành phần trên quy trình Pha 2 --- chỉ huấn luyện lại tầng phân loại trên đặc trưng đã trích sẵn trong khi mạng xương sống được đóng băng --- theo một ma trận đối chứng có kiểm soát. Lưu ý ma trận này chạy trên tập con siêu dữ liệu dùng cho kiểm định chéo với hệ số nhiệt độ riêng, nên F1 tuyệt đối không so trực tiếp được với kết quả chính ở Mục~\ref{sec:extra_param_sweep}; bảng chỉ nhằm đọc mức chênh $\Delta$F1 giữa các cấu hình thành phần.

\begin{table}[H]
\centering
\caption[Phân rã thành phần có kiểm soát trên Pha 2]{Phân rã thành phần có kiểm soát trên Pha 2: $\Delta$F1 so với đối chứng A1 (điểm phần trăm). Số đậm đánh dấu cấu hình $B4$ được chọn làm cấu hình hậu xử lý chính.}
...
\end{tikzpicture}
\caption[So sánh F1 trên tập SHOT]{So sánh F1 giữa các mô hình trên tập SHOT (200 video kiểm tra). AutoShotV2 đạt F1 = \ASVTwoBestShotFOne{} theo lựa chọn tốt nhất trong giao thức so sánh chính, cải thiện $+\ASVTwoBestVsAutoShotShotPP{}$ điểm phần trăm so với AutoShot tự chạy lại và $+\ASVTwoBestVsTransNetShotPP{}$ điểm phần trăm so với TransNetV2 mô hình cơ sở.}
