...
Video được tổ chức theo cấu trúc phân cấp từ lớn đến nhỏ: cảnh chứa nhiều cảnh quay, và mỗi cảnh quay là tập hợp các khung hình liên tiếp được ghi liền mạch. Phát hiện ranh giới cảnh quay là việc xác định các vị trí trong dòng khung hình nơi một cảnh quay kết thúc và một cảnh quay mới bắt đầu. Tác vụ này được thực hiện ở mức khung hình: nếu không có thay đổi đáng kể giữa các khung hình liên tiếp, không có ranh giới cảnh quay. Với chuyển cảnh dần, hệ thống thường phải quan sát đồng thời nhiều khung hình kế tiếp để xác nhận vì quá trình chuyển diễn ra trải dài theo thời gian.
...
\begin{itemize}
    \item \textbf{Chuyển cảnh đột ngột:} Là sự thay đổi tức thì giữa hai shot mà không có hiệu ứng chỉnh sửa nào ở giữa, xảy ra khi hai shot được nối trực tiếp. Đây là loại chuyển cảnh phổ biến và tương đối dễ phát hiện do sự thay đổi đột ngột về nội dung hình ảnh trong một khung hình.

    \begin{figure}[htbp]
        \centering
        \includegraphics[width=0.6\textwidth]{images/cut_transition.png}
        \caption{Minh họa chuyển cảnh đột ngột.}
        \label{fig:cut_transition}
    \end{figure}
    \FloatBarrier

    \item \textbf{Chuyển cảnh dần dần:} Là sự kết hợp giữa hai shot thông qua các hiệu ứng đồ họa, trải dài trên nhiều khung hình. Các dạng phổ biến gồm hiệu ứng mờ dần sang đen, xuất hiện dần từ nền đen, chồng hình và quét hình. Trong đó, quét hình khó phát hiện hơn do có nhiều hình thức thể hiện. Xem Hình~\ref{fig:shot_transitions} để so sánh trực quan.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Nhiễu thị giác:} Các yếu tố như ánh sáng lóe lên đột ngột, chuyển động nhanh của vật thể hoặc chuyển động lia hoặc thu phóng của máy quay và nền tương tự nhau giữa hai shot kế tiếp dễ bị nhầm lẫn với chuyển cảnh thật.
    \begin{figure}[htbp]
        \centering
        \includegraphics[width=0.6\textwidth]{images/flash.jpg}
        \caption{Nhiễu thị giác do hiện tượng lóe sáng đột ngột.}
        \label{fig:visual_noise_flash}
    \end{figure}
    \FloatBarrier

    \item \textbf{Khó khăn khi phát hiện chuyển cảnh dần:} Không giống chuyển cảnh đột ngột, chuyển cảnh dần diễn ra từ từ và có đặc điểm thống kê không rõ ràng. Các phương pháp dựa trên chênh lệch pixel hoặc histogram thường thất bại khi gặp các hiệu ứng chồng hình hoặc quét hình phức tạp.
...
\begin{itemize}
    ...
    \item \textbf{Mục tiêu đầu ra:} Bài toán tập trung vào việc xác định vị trí biên của các shot; chưa đi sâu vào việc hiểu ngữ nghĩa mức cao (như nhận diện hành động hay cảm xúc) bên trong shot.
...
\begin{itemize}
    ...
    \item \textbf{RQ2:} Các kỹ thuật huấn luyện và hậu xử lý ở tầng phân loại, gồm hàm mất mát Focal, nhãn nhiều điểm dương, hiệu chỉnh nhiệt độ và làm mượt Gaussian, có thể cải thiện F1 mà không cần thay đổi mạng xương sống hay không?
...
\begin{itemize}
    ...
    \item \textbf{Chương 4 -- Bộ dữ liệu và thực nghiệm:} Mô tả các bộ dữ liệu (SHOT, BBC, ClipShots), chỉ số đánh giá, mô hình cơ sở, siêu tham số, kịch bản thực nghiệm, kết quả so sánh, phân tích độ chính xác dương, độ bao phủ, chỉ số F1 và thực nghiệm phân rã.
