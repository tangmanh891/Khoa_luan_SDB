...
\begin{itemize}
    ...
    \item \textbf{Đóng góp về kết quả thực nghiệm:} Trên tập SHOT, AutoShotV2 đạt F1 = \ASVTwoBestShotFOne{} ở ngưỡng cho F1 cao nhất theo đúng giao thức đánh giá của AutoShot gốc ($+\ASVTwoBestVsTransNetShotPP{}$ điểm phần trăm so với mô hình cơ sở TransNetV2 và $+\ASVTwoBestVsAutoShotShotPP{}$ điểm phần trăm so với AutoShot tự chạy lại); đồng thời cung cấp phân tích so sánh chéo trên BBC và ClipShots để đánh giá khả năng tổng quát hóa (chi tiết xem Chương~\ref{ch:bo_du_lieu_va_thuc_nghiem}).
\end{itemize}
