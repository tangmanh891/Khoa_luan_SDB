...
Kết nối & Tuần tự & Kết nối tắt sau mỗi 2 khối \\
Hạt nhân tích chập & 3D đầy đủ & Phân tách không gian–thời gian \\
Nhánh bổ sung & Không có & Learned + Histogram similarity \\
Đầu ra & Một đầu phân loại & Hai đầu phân loại (single + all frame) \\
...
Mỗi thay đổi trong Bảng~\ref{tab:transnetv1_v2} giải quyết một vấn đề cụ thể. BatchNorm và kết nối tắt ổn định quá trình huấn luyện khi mô hình sâu hơn. Phân tách hạt nhân giảm nguy cơ quá khớp trên dữ liệu nhân tạo có phân phối hạn chế. Nhánh đặc trưng tương đồng cung cấp tín hiệu bổ sung không thể học được chỉ từ tích chập cục bộ. Hai đầu phân loại buộc mô hình phân biệt giữa "tại đâu là ranh giới" và "vùng nào là chuyển cảnh", hai câu hỏi bổ sung cho nhau. Cuối cùng, bổ sung dữ liệu thực vào 15\% tập huấn luyện thu hẹp khoảng cách phân phối giữa huấn luyện và kiểm thử.
...
Nhờ cấu trúc giãn nở, trường nhìn thời gian (temporal receptive field) tại khối DDCNN thứ $k$ có thể được tính theo quy nạp. Với hạt nhân thời gian kích thước $K = 3$ và 4 nhánh dilation $\{1, 2, 4, 8\}$, trường nhìn cộng dồn sau $k$ khối là:
\begin{equation}
...
Thay vì dùng hạt nhân tích chập 3D đầy đủ kích thước $k \times k \times k$, TransNetV2 phân tách mỗi phép tích chập thành hai bước tuần tự nhằm giảm số tham số và buộc mạng học riêng biệt hai loại thông tin:
\begin{enumerate}
    \item \textbf{Tích chập không gian 2D} với hạt nhân $1 \times k \times k$: xử lý từng khung hình trong chuỗi một cách độc lập theo hai chiều không gian (height, width), học đặc trưng nội dung thị giác như cạnh, kết cấu, vùng màu.
    \item \textbf{Tích chập thời gian 1D giãn nở} với hạt nhân $k \times 1 \times 1$ và độ giãn nở $d$: xử lý chuỗi đặc trưng theo trục thời gian, học sự biến đổi chuyển động và chuyển cảnh.
\end{enumerate}

Phân tách giảm số tham số còn $\approx 44\%$ so với hạt nhân 3D đầy đủ: với $k=3$ và $C_{in}=C_{out}=C_{mid}=C$, tỉ lệ là $(9C^2+3C^2)/(27C^2)=12/27\approx44\%$, tức tiết kiệm $\approx56\%$ tham số bất kể số kênh. Ngoài lợi ích về bộ nhớ, việc ép xử lý không gian trước rồi mới xử lý thời gian tạo ra thiên kiến quy nạp phù hợp với SBD: nội dung khung thay đổi theo không gian, còn chuyển cảnh là hiện tượng theo thời gian — hai biến đổi nên được mô hình hóa riêng biệt.
...
Khối Transformer (vị trí 7) & \textbf{Mới} & Tùy chọn tầng tự chú ý \\
...
Bảng~\ref{tab:block_variants} là nguồn so sánh chính giữa bốn biến thể; điểm cốt lõi cần ghi nhớ là không gian tìm kiếm phủ hai trục thiết kế: \textit{cách chia sẻ tích chập không gian} (riêng biệt như V2 vs. dùng chung như V2A, V2C) và \textit{cách ghép không gian--thời gian} (Concat như V2/V2A vs. Add lấy cảm hứng P3D như V2B/V2C). Đưa cả hai cơ chế Concat và Add vào không gian tìm kiếm cho phép NAS tự cân nhắc giữa tính phân tách thông tin (Concat) và sự ổn định gradient kiểu ResNet (Add) phù hợp nhất với từng vị trí trong mạng xương sống của video ngắn.
...
Khối Transformer thứ 7 có 5 tùy chọn về số lớp tự chú ý: $\{0, 1, 2, 3, 4\}$. Các 6 khối tích chập được cấu hình độc lập, nên tổng số kiến trúc trong không gian tìm kiếm là:
\begin{equation}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[>=Latex,font=\footnotesize]
        ...
        \node[draw, rounded corners, fill=purple!12, text width=3.0cm, minimum height=1.25cm, align=center] (p4) at (12.4,0) {(4) Huấn luyện lại\\kiến trúc $\alpha^*$};
...
\end{equation}
trong đó $\alpha^{(n)}$ là kiến trúc được lấy mẫu tại bước $n$, $\mathcal{B}^{(n)}$ là lô dữ liệu (batch), và $\mathcal{L}$ là hàm mất mát giống phương trình~\eqref{eq:transnetv2_loss}.

Chiến lược lấy mẫu đồng đều đảm bảo mọi kiến trúc con được huấn luyện với xác suất bằng nhau, tránh thiên lệch về một loại khối nhất định. Điều này quan trọng để trọng số chia sẻ $\theta_{\mathrm{super}}$ phản ánh khả năng chung của tất cả kiến trúc, không bị áp đảo bởi một số ít kiến trúc chiếm ưu thế. Sau khi huấn luyện 12 chu kỳ (epoch), $\theta_{\mathrm{super}}$ đóng vai trò oracle đánh giá nhanh: cho bất kỳ kiến trúc $\alpha$, điểm F1 ước tính được tính bằng cách nạp $\theta_{\mathrm{super}}$ và suy diễn trên tập xác thực mà không cần huấn luyện lại.
...
\midrule
1. Huấn luyện SuperNet & 12 chu kỳ, một đường đơn & 8× V100 & $\approx$ 12--16 giờ \\
2. Tìm kiếm kiến trúc & 4800 đánh giá (GP + PoF) & 8× V100 & $\approx$ 40--50 giờ \\
3. Huấn luyện lại (Retraining) & 12 chu kỳ với $\alpha^*$ & 1× V100 & $\approx$ 12 giờ \\
4. Chưng cất tri thức (KD) & 12 chu kỳ, học sinh học từ giáo viên & 1× V100 & $\approx$ 12 giờ \\
...
Chưng cất tri thức (KD) \cite{hinton2015distilling} sử dụng mô hình ứng viên tốt nhất sau bước huấn luyện lại làm \textit{mô hình giáo viên}, hướng dẫn quá trình học của \textit{mô hình học sinh} — cũng là kiến trúc $\alpha^*$ nhưng khởi tạo ngẫu nhiên. Ý tưởng cốt lõi: thay vì học từ nhãn cứng nhị phân $\{0, 1\}$, học sinh học từ nhãn mềm của giáo viên.

Nhãn mềm $\tilde{y}^{(j)}_{i,t}$ là xác suất dự đoán của giáo viên tại khung $t$ của chuỗi $i$, nhánh $j \in \{1, 2\}$. Hàm mất mát KD:
\begin{equation}
    ...
    \label{eq:kd_loss}
\end{equation}
trong đó $\tilde{y}^{(j)}$ là nhãn mềm của giáo viên (cố định, không có gradient qua giáo viên), $\hat{y}^{(j)}$ là dự đoán của học sinh, và $\lambda_1 = 5.0$, $\lambda_2 = 0.1$ như trong phương trình~\eqref{eq:transnetv2_loss}.

So sánh với phương trình~\eqref{eq:transnetv2_loss}, sự khác biệt duy nhất là nhãn chuẩn $y^{(j)}$ được thay bằng nhãn mềm $\tilde{y}^{(j)}$ từ giáo viên. Nhãn mềm mang nhiều thông tin hơn nhãn nhị phân: chúng phản ánh độ không chắc chắn của giáo viên, đặc biệt tại các khung hình thuộc vùng chuyển cảnh dần hay các chuyển cảnh đột ngột có nền màu tương đồng. Nhờ đó, học sinh học được phân phối xác suất trơn tru hơn, ít kích hoạt cực đoan hơn và bền vững hơn trước các biến thể rìa ranh giới — đây là cốt lõi của "giảng dạy về sự không chắc chắn" trong KD.

Trong AutoShot, giáo viên và học sinh có \textit{cùng kiến trúc} $\alpha^*$ — đây là trường hợp KD tự học (self-distillation). Khác với KD truyền thống (giáo viên lớn hơn học sinh), tự học giúp học sinh thoát khỏi cực trị cục bộ mà bước huấn luyện lại trước đó có thể đã rơi vào, do điểm khởi đầu ngẫu nhiên khác nhau.
...
Cho hai mô hình $M_1$ (sau huấn luyện lại) và $M_2$ (sau KD) với trọng số lớp $l$ lần lượt là $W_1^{(l)}$ và $W_2^{(l)}$, entropy được ước tính bằng histogram $B = 10$ bin trên phân phối của các giá trị trọng số:
\begin{equation}
...
Nhóm tác giả tiến hành tìm kiếm hai lần với hai tiêu chí đánh giá khác nhau, thu được hai kiến trúc có đặc điểm bổ sung cho nhau: AutoShot@F1 tối ưu cân bằng độ chính xác dương và độ bao phủ, còn AutoShot@Precision ưu tiên giảm dương tính giả.
...
Kỹ thuật sau tìm kiếm & Huấn luyện trực tiếp & Huấn luyện lại, chưng cất tri thức và Weight Grafting \\
...
Độ chính xác dương trên SHOT & Khoảng 80\% & \textbf{93.9\%} với AutoShot@Precision \\
\bottomrule
\end{tabularx}
\end{table}
...
AutoShot@F1 đạt F1 = 0.8405 với BCE chuẩn và nhãn một điểm dương. Tầng phân loại vẫn còn hai hạn chế. Thứ nhất, dữ liệu mất cân bằng lớp nghiêm trọng do các khung hình ranh giới chiếm dưới 1\% tổng số khung hình. Thứ hai, tín hiệu nhãn cứng nhắc không dung nạp sai lệch chú thích. AutoShotV2 áp dụng chiến lược \textbf{đóng băng mạng xương sống + huấn luyện lại tầng phân loại}: khóa toàn bộ trọng số AutoShot@F1, huấn luyện bộ phân lớp mới từ đầu trên các đặc trưng đã được lưu bộ nhớ đệm. Quyết định này dựa trên ba lý do thực tế: NAS đã tối ưu mạng xương sống với chi phí khoảng 76--91 giờ trên 8$\times$V100 theo Bảng~\ref{tab:autoshot_complexity}; việc đóng băng mạng xương sống giúp cô lập đóng góp của tầng phân loại; và chỉ huấn luyện tầng phân loại trên đặc trưng đã lưu vào bộ nhớ đệm giúp giảm chi phí huấn luyện lại từ hàng chục giờ xuống còn vài chục phút trên một GPU phổ thông.

Khóa luận trình bày cải tiến theo hai bước tăng dần. \textbf{AutoShotV1} là bước tối giản nhất: giữ nguyên toàn bộ trọng số của AutoShot@F1, bao gồm mạng xương sống và tầng phân loại và chỉ bổ sung làm mượt Gaussian ở khâu hậu xử lý --- không huấn luyện lại bất kỳ tham số nào, chỉ dò hệ số $\sigma$ trên tập xác thực. \textbf{AutoShotV2} là phiên bản đầy đủ: đóng băng mạng xương sống nhưng huấn luyện lại tầng phân loại với hàm mất mát Focal và nhãn nhiều điểm dương, kết hợp hiệu chỉnh nhiệt độ và làm mượt Gaussian ở hậu xử lý. Kết quả định lượng của AutoShotV1 được trình bày ở Mục~\ref{sec:autoshotv1}; phần còn lại của mục này tập trung mô tả AutoShotV2.

\subsection{Kiến trúc tầng phân loại}
...
\begin{enumerate}
    ...
    \item \textbf{cls\_layer1}: Linear(1024 $\to$ 1) --- nhánh chính với \textbf{nhãn một điểm dương}, học định vị ranh giới chính xác.
    \item \textbf{cls\_layer2}: Linear(1024 $\to$ 1) --- nhánh phụ với \textbf{nhãn nhiều điểm dương}, học ngữ cảnh vùng quanh ranh giới.
\end{enumerate}

Toàn bộ đặc trưng mạng xương sống được \textbf{tính trước và lưu vào bộ nhớ đệm} trong file \texttt{.pickle} trước khi huấn luyện, giúp mỗi chu kỳ huấn luyện chỉ cần chạy qua bộ phân lớp nhỏ thay vì toàn bộ mạng xương sống 55 MB, tăng tốc đáng kể vòng lặp huấn luyện.
...
\end{tikzpicture}
\caption[Kiến trúc tầng phân loại trong AutoShotV2]{Kiến trúc tầng phân loại trong AutoShotV2. Tầng phân loại này nhận đặc trưng 4864-D từ mạng xương sống đã đóng băng và sinh hai đầu ra song song: nhánh một điểm dương định vị ranh giới chính xác và nhánh nhiều điểm dương học ngữ cảnh vùng biên.}
...
\end{equation}
Đặt $p_t$ là xác suất gán cho lớp đúng ($p_t = p$ nếu $y=1$; $p_t = 1-p$ nếu $y=0$), BCE viết gọn thành $\mathcal{L}_{\text{BCE}} = -\log(p_t)$. Khi tổng hợp giá trị mất mát trên toàn bộ lô, các mẫu âm dễ (khung không có chuyển cảnh, rất dễ phân loại) áp đảo về số lượng và kéo gradient theo hướng ``luôn dự đoán không có ranh giới''.
...
Tham số $\gamma \in \{1.0, 2.0\}$ và $\alpha \in \{0.5, 0.75\}$ được chọn qua \textbf{tìm kiếm theo lưới} (8 tổ hợp), đánh giá theo F1 trên tập xác thực. Cấu hình tốt nhất: $\boldsymbol{\gamma = 1.0}$, $\boldsymbol{\alpha = 0.5}$. Khác với BCE có trọng số (chỉ điều chỉnh trọng số tĩnh giữa hai lớp) hoặc hàm mất mát cân bằng lớp (dựa trên số mẫu hữu hiệu), Focal Loss điều chỉnh \textit{động} theo độ khó của từng mẫu --- phù hợp với SBD nơi mức độ khó biến thiên mạnh giữa chuyển cảnh đột ngột, chuyển cảnh dần và các trường hợp ánh sáng đột biến. Trong tìm kiếm theo lưới, $\gamma=1.0$ thắng $\gamma=2.0$ do nhánh nhiều điểm dương (Mục~\ref{subsec:v2_manyhot}) đã cung cấp thêm tín hiệu giám sát cho vùng biên, nên hệ số làm sắc quá mạnh khiến gradient ở các khung biên bị triệt tiêu sớm.

\subsection{Nhãn nhiều điểm dương --- giám sát vùng biên}
\label{subsec:v2_manyhot}

Nhãn một điểm dương chuẩn chỉ gán giá trị 1 tại đúng một khung hình ranh giới và 0 cho tất cả còn lại. Cách biểu diễn này chính xác về định vị, nhưng quá cứng nhắc trong thực tiễn SBD vì hai lý do: \textbf{(i)} chú thích thủ công có thể lệch $\pm1$ frame so với ranh giới thực sự; \textbf{(ii)} chuyển cảnh dần kéo dài trên nhiều frame, nên các khung lân cận ranh giới cũng mang tín hiệu chuyển cảnh đáng kể.

AutoShotV2 bổ sung một nhánh giám sát phụ với \textbf{nhãn nhiều điểm dương}~\cite{Muller2019LabelSmoothing}: tất cả các khung trong phạm vi $\pm1$ khung hình quanh ranh giới đều được coi là dương:
\begin{equation}
    y_{\text{many-hot}}[t] =
    \begin{cases}
...
Nhánh một điểm dương giữ vai trò chính: định vị ranh giới sắc nét tại đúng thời điểm chuyển cảnh. Nhánh nhiều điểm dương đóng vai trò phụ với hệ số 0.3: dạy mô hình rằng các khung lân cận ranh giới là những mẫu ``gần đúng'' và không nên bị phạt nặng khi dự đoán lệch nhẹ. Hệ số 0.3 được chọn nhỏ hơn 1 để nhánh phụ hỗ trợ mà không lấn át định vị chính xác.

Bán kính $\pm 1$ khung hình được chọn căn cứ vào phân phối sai số chú thích thực tế trên SHOT: phần lớn ranh giới thủ công được đánh lệch không quá một khung so với chuyển cảnh đột ngột thật, và chuyển cảnh dần thường kéo dài 3--5 khung sao cho ba khung trung tâm $\{t-1, t, t+1\}$ mang tín hiệu chuyển cảnh rõ nhất. Mở rộng sang $\pm 2$ khung hình trong thử nghiệm sơ bộ làm tăng nhiễu giám sát vì các khung biên xa hơn, chẳng hạn $t \pm 2$, thường đã thuộc cảnh quay lân cận, gây nhầm chuyển cảnh đột ngột với chuyển cảnh dần và làm giảm độ chính xác dương.
...
\end{tikzpicture}
\caption[Minh họa nhãn một điểm dương và nhiều điểm dương]{Minh họa nhãn một điểm dương và nhãn nhiều điểm dương. Nhãn một điểm dương chỉ gán 1 tại đúng khung ranh giới $t$; nhãn nhiều điểm dương mở rộng thêm $t\pm1$, cung cấp tín hiệu gradient dày đặc hơn quanh vùng ranh giới.}
\label{fig:v2_onehot_manyhot}
\end{figure}
\FloatBarrier

\subsection{Làm mượt Gaussian --- triệt tiêu nhiễu đơn lẻ}
\label{subsec:v2_gaussian}

Sau khi mô hình sinh chuỗi xác suất $p[t]$, tín hiệu này thường chứa các đỉnh nhọn hẹp do nhiễu cục bộ: một khung đơn lẻ có xác suất cao bất thường vì lóe sáng, rung máy, hoặc biến đổi màu sắc đột ngột. Nếu ngưỡng hóa trực tiếp, các đỉnh nhiễu dễ bị nhận nhầm là ranh giới cảnh. AutoShotV2 áp dụng \textbf{làm mượt Gaussian 1D} với $\sigma = 2.0$ lên chuỗi xác suất trước bước ngưỡng hóa:
\begin{equation}
...
Nguyên tắc hoạt động: ranh giới thật tạo phản hồi nhất quán trên vài khung liên tiếp --- sau khi làm mượt, các khung ``hỗ trợ lẫn nhau'' và đỉnh xác suất được giữ lại. Nhiễu đơn lẻ không được hàng xóm ủng hộ bị kéo xuống đáng kể. Tham số $\sigma = 2.0$ được chốt sau khi quét $\sigma \in \{1.0, 1.5, 2.0, 2.5, 3.0\}$ trên tập xác thực của SHOT: giá trị nhỏ hơn ($\sigma \le 1.5$) chưa đủ triệt nhiễu lóe sáng, trong khi giá trị lớn hơn ($\sigma \ge 2.5$) làm phẳng cả các đỉnh ranh giới thật và đẩy ngưỡng tối ưu xuống vùng quá nhỏ, kém ổn định. So với bộ lọc trung vị (median filter) --- một lựa chọn hậu xử lý phổ biến khác --- hạt nhân Gaussian giữ được dạng đỉnh khi ranh giới được nhiều khung liên tiếp hỗ trợ và tránh hiện tượng cắt phẳng đột ngột, phù hợp hơn với phân phối xác suất sigmoid của tầng phân loại.

\subsection{Hiệu chỉnh nhiệt độ (Temperature Scaling) --- hiệu chỉnh xác suất}
\label{subsec:v2_temperature}

Sau huấn luyện, phân phối xác suất đầu ra của mô hình có thể chưa được hiệu chỉnh tốt: mô hình có thể thiếu tự tin, nghĩa là xác suất bị nén về gần 0,5 dù nhiều mẫu đã được phân loại khá chắc chắn, hoặc quá tự tin. Trong SBD, hiệu chỉnh xác suất trực tiếp ảnh hưởng đến việc chọn ngưỡng tối ưu.
...
Trên tập SHOT, hệ số tối ưu tìm được thỏa $T^* < 1$, cho thấy mô hình đang \textit{thiếu tự tin} --- nhiều dự đoán đúng nhưng xác suất chưa đủ ``dứt khoát''. Chia logit cho $T^* < 1$ làm sắc hóa phân phối, đẩy các dự đoán rõ ràng ra xa vùng trung tính. Hệ quả: ngưỡng quyết định tối ưu dịch về vùng nhỏ hơn so với ngưỡng của AutoShot gốc; giá trị $T^*$ và ngưỡng cụ thể của từng cấu hình được báo cáo ở Chương~\ref{ch:bo_du_lieu_va_thuc_nghiem}.

Khoảng tìm kiếm $T \in [0.1, 10.0]$ được chọn theo khuyến nghị của Guo et al.\ \cite{Guo2017Calibration} cho bài toán phân loại nhị phân: cận dưới $0.1$ cho phép sắc hóa rất mạnh khi mô hình thiếu tự tin, còn cận trên $10.0$ phủ trường hợp quá tự tin mà không gặp rủi ro chia cho giá trị quá lớn làm gradient triệt tiêu. So với hiệu chỉnh Platt (Platt scaling - học hai tham số affine $a, b$ trên logit) hoặc hồi quy đơn điệu (isotonic regression - hàm bậc thang không tham số), hiệu chỉnh nhiệt độ chỉ có một tham số duy nhất nên ít có nguy cơ quá khớp trên tập xác thực nhỏ và giữ nguyên thứ tự xếp hạng dự đoán --- một tính chất then chốt để ngưỡng hóa nhị phân của SBD vẫn ổn định sau hiệu chỉnh.

\subsection{Quy trình huấn luyện lại và thuật toán tổng thể}
\label{subsec:v2_training}

Tầng phân loại được huấn luyện lại theo chiến lược hai pha~\cite{Loshchilov2016SGDR}: khởi động tuyến tính trong 4 chu kỳ huấn luyện, với tốc độ học tăng từ 0 đến $10^{-5}$, sau đó giảm tốc độ học theo hàm cosin trong tổng số tối đa 20 chu kỳ huấn luyện. Cấu hình chạy cuối không dùng cơ chế dừng sớm, nhằm giữ cùng ngân sách huấn luyện cho các biến thể thực nghiệm phân rã.

\section{Tổng kết chương}
\label{sec:chap3_conclusion}

Chương này đã trình bày hướng tiếp cận kế thừa AutoShot và cải tiến ở tầng đầu ra để hình thành AutoShotV2. Các nội dung chính gồm nền tảng NAS và TransNetV2, mô hình AutoShot gốc, kiến trúc tầng phân loại mới, hàm mất mát Focal, nhãn nhiều điểm dương, làm mượt Gaussian, hiệu chỉnh nhiệt độ và quy trình huấn luyện lại. Chương~\ref{ch:bo_du_lieu_va_thuc_nghiem} tiếp theo mô tả dữ liệu, thiết lập thực nghiệm và kết quả đánh giá.
