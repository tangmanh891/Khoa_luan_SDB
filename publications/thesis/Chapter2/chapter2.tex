...
\section{Tổng quan bài toán phát hiện ranh giới cảnh quay}
...
Để giảm chi phí giải nén toàn bộ video, các phương pháp trên miền nén (\textit{compressed-domain}) khai thác trực tiếp dữ liệu trong dòng bit nén (ví dụ vectơ chuyển động và các hệ số biến đổi cosin rời rạc DCT của chuẩn MPEG) \cite{Kar2024_VideoSBD_IssuesChallengesSolutions}. Cách tiếp cận tương tự là dùng dòng quang học (\textit{optical flow}) để ước lượng chuyển động giữa hai khung hình: nếu trường vectơ dòng quang học thay đổi đột ngột về phân bố, khả năng cao đó là một ranh giới shot. Các phương pháp này tiết kiệm chi phí tính toán nhưng độ chính xác phụ thuộc mạnh vào chất lượng tín hiệu chuyển động được nén và thường yêu cầu tinh chỉnh ngưỡng theo từng bộ mã hóa-giải mã.
...
\begin{enumerate}
    \item \textbf{Độ phụ thuộc ngưỡng cao:} Mỗi tập dữ liệu hoặc thậm chí từng video cần một ngưỡng riêng để cân bằng độ chính xác dương và độ bao phủ. Trong video ngắn với nhịp cắt dày và phong cách hậu kỳ đa dạng, một ngưỡng cố định khó đạt hiệu quả tổng quát.
...
TransNet \cite{soucek2019transnet} và TransNetV2 \cite{soucek2020transnetv2} là các mô hình cơ sở mạnh trong SBD hiện đại. TransNetV2 khai thác các khối tích chập giãn cho phép tăng trường tiếp nhận thời gian mà không tăng số tham số tương ứng. Cùng với kỹ thuật phân tách hạt nhân tích chập, trong đó phép tích chập ba chiều được tách thành tích chập không gian hai chiều và tích chập thời gian một chiều, TransNetV2 đạt mức cân bằng tốt giữa độ chính xác và chi phí tính toán.
...
\begin{itemize}
    ...
    \item \textbf{Hai đầu phân loại song song:} Một đầu dự đoán xác suất chuyển cảnh tại một khung đơn (single-frame), một đầu dự đoán xác suất ``thuộc về một chuyển cảnh dần'' (all-frame). Hai tín hiệu này được kết hợp ở giai đoạn suy diễn để vừa định vị chính xác vị trí chuyển cảnh đột ngột, vừa bao quát được toàn bộ vùng chuyển cảnh dần.
\end{itemize}
...
Sau thành công của Transformer \cite{vaswani2017attention} trong xử lý chuỗi, nhiều nghiên cứu SBD gần đây áp dụng cơ chế tự chú ý để mô hình hóa phụ thuộc xa giữa các khung hình. Esteve và cộng sự \cite{esteve2023shot} đề xuất một mô hình kết hợp tích chập chiều sâu ba chiều với cơ chế chú ý thị giác, cho thấy cơ chế chú ý có thể bổ trợ tốt cho phép tích chập trong việc phát hiện các chuyển cảnh dần dài. Theo cùng hướng, nhiều công trình khảo sát trong các tổng kết gần đây \cite{Kar2024_VideoSBD_IssuesChallengesSolutions} cho thấy xu hướng dùng kết hợp đa phương thức (kết hợp đặc trưng hình ảnh với âm thanh hoặc tín hiệu chuyển động) để tăng tính bền vững khi chuyển miền dữ liệu.
...
\begin{table}[htbp]
    ...
    \footnotesize
    \begin{tabularx}{\textwidth}{L{2.7cm} L{2.6cm} L{2.8cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
        \toprule
        \textbf{Nhóm phương pháp} & \textbf{Loại mô hình} & \textbf{Bộ dữ liệu thường dùng} & \textbf{Ưu điểm} & \textbf{Hạn chế} \\
        \midrule
        Đặc trưng thủ công & Biểu đồ màu (Histogram), chênh lệch điểm ảnh, dựa trên biên cạnh, trên miền nén & BBC và các tập video truyền thống & Đơn giản, dễ triển khai, chi phí thấp & Phụ thuộc ngưỡng, dễ nhiễu bởi ánh sáng/chuyển động, yếu với chuyển cảnh dần phức tạp \\
