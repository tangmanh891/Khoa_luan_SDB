...
presents the AutoShotV2 decision-stage pipeline built on top of it. Note that
the convolutional blocks and search spaces are inherited from prior work;
AutoShotV2 modifies only the decision stage described from Section~\ref{sec:head} onward.
...
\begin{equation}
    ...
    \label{eq:threshold}
\end{equation}
where $\theta$ is a fixed operating point selected before test evaluation. Following standard practice,
...
Fig.~\ref{fig:pipeline} shows the AutoShotV2 inference pipeline.
...
\node[block, fill=purple!12, right=of head] (temp) {Temperature scaling\\$T^{*}=0.2413$};
\node[block, fill=pink!14, right=of temp] (smooth) {Gaussian smoothing\\$\sigma=\PaperGaussianSigma$};
\node[block, fill=blue!8, right=of smooth] (out) {Threshold\\$\theta=0.12$ (SHOT)};
\node[block, fill=teal!12, right=of out] (bound) {Shot boundaries\\F1 $=0.8607$ (SHOT)};
...
$\gamma=2.0$ and $\alpha=0.6$ for the primary configuration. Second, boundary-aware many-hot
...
\begin{equation}
    ...
    \label{eq:gaussian}
\end{equation}
The primary configuration fits $T^{*}=0.2413$ on its validation pool and
selects the decision threshold per dataset under AutoShot's protocol
(Section~\ref{sec:oracle_sweep}); the tracked deployed-checkpoint anchor
instead uses $T^{*}=\PaperDeployTemperature$ with a single shared
$\theta=\PaperDeployThreshold$, and the controlled replication re-fits its own
temperature under the frozen protocol below. All use $\sigma=\PaperGaussianSigma$.

\subsection{Validation-Only Protocol Selection}
\label{sec:protocol}

For the controlled track, we select the complete post-processing configuration using
