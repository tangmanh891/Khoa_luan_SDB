...
Following AutoShot's original evaluation protocol, which selects the per-dataset F1-maximizing decision threshold, AutoShotV2 achieves F1 scores of 0.8607 on SHOT, 0.9656 on BBC, and 0.7706 on ClipShots, outperforming the reproduced AutoShot baseline by 2.02, 1.02, and 0.57 percentage points, respectively, under the same procedure. Because both models are scored with this shared threshold-selection protocol, the comparison is direct rather than an optimistic upper bound. A controlled replication under a frozen, validation-only five-fold protocol separates training choices from post-processing choices: the training-loss alternatives change F1 only marginally, whereas calibrated post-processing adds \PaperBFourVsAOneShotPP{} percentage points on SHOT and \PaperBFourVsAOneClipPP{} percentage points on ClipShots in F1 over the BCE control. Paired bootstrap intervals support the positive deltas on SHOT and ClipShots, but not on BBC.

In summary, our main contributions are:
\begin{itemize}
\item We propose AutoShotV2, an efficient frozen-feature SBD refinement pipeline that improves reproduced AutoShot under AutoShot's original per-dataset best-threshold protocol, reaching F1 scores of 0.8607 on SHOT, 0.9656 on BBC, and 0.7706 on ClipShots.
\item We analyze cross-domain generalization on long-form BBC documentary footage and open-world ClipShots, and release a tracked, checksummed fixed-threshold checkpoint (0.8545, 0.9656, and 0.7529 F1) as a deployment and reproducibility anchor.
...
For reproducibility, the paper maintains two artifact tracks. A fixed-threshold deployed checkpoint is preserved through tracked, checksummed result files and provides a deployment-robustness snapshot. A controlled replication with full logit caches anchors the logit-level confidence intervals, calibration diagnostics, ablations, and seed study (Section~\ref{sec:tracks}).
