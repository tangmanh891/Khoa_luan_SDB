...
With a fixed deployment configuration, AutoShotV2 achieves an F1 score of \PaperDeployShotFOne{} on the SHOT test set, improving over the reproduced AutoShot baseline by \PaperDeployVsAutoShotPP{} percentage points in F1 (a supplemental configuration with a different checkpoint reaches 0.8607 under oracle per-dataset threshold; see Section~\ref{sec:extra_param_sweep}). A controlled replication under a frozen,
validation-only
