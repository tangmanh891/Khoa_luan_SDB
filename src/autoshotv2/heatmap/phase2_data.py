...
def build_sample_cache_config(
    ...
    return {
        ...
        "heatmap_sigma": getattr(args, "heatmap_sigma", 0.0),
        "heatmap_mode": getattr(args, "heatmap_mode", "gaussian"),
...
def make_heatmap_labels(
    ...
    """Tạo nhãn mềm dạng heatmap xung quanh mỗi frame biên giới.

    Thay vì nhãn cứng (0 hoặc 1), mỗi frame nhận giá trị trong [0, 1]
    phụ thuộc vào khoảng cách đến boundary gần nhất:

      gaussian:   exp(-0.5 * (d/sigma)^2)           — đỉnh nhọn, giảm nhanh
      triangular: max(0, 1 - d / (sigma * truncate)) — giảm tuyến tính
      cosine:     0.5*(1 + cos(pi*d/(sigma*truncate))) nếu d < sigma*truncate

    Tại frame biên giới chính xác (d=0): label = 1.0.
    Ngoài vùng sigma*truncate: label = 0.0.

    Args:
        one_hot:  nhãn cứng 0/1 gốc (1 tại boundary frame)
        sigma:    độ rộng phân bố tính bằng frame (e.g. 2.0)
        mode:     "gaussian" | "triangular" | "cosine"
        truncate: số sigma để cắt về 0 (default 3.0)
    """
    n = len(one_hot)
    labels = np.zeros(n, dtype=np.float32)
    boundary_frames = np.flatnonzero(one_hot > 0)
    ...
    for b in boundary_frames:
        lo = max(0, b - radius)
        hi = min(n, b + radius + 1)
        d = np.abs(np.arange(lo, hi) - b).astype(np.float32)
        if mode == "gaussian":
            vals = np.exp(-0.5 * (d / sigma) ** 2)
        elif mode == "triangular":
            vals = np.maximum(0.0, 1.0 - d / (sigma * truncate))
        elif mode == "cosine":
            vals = np.where(
                d < sigma * truncate,
                0.5 * (1.0 + np.cos(np.pi * d / (sigma * truncate))),
                0.0,
            ).astype(np.float32)
        else:
            raise ValueError(f"Unknown heatmap mode: {mode!r}. Choose gaussian/triangular/cosine.")
        # element-wise max để handle overlapping boundaries
        labels[lo:hi] = np.maximum(labels[lo:hi], vals)
    ...
    try:
        for i, key in enumerate(work_keys, 1):
            ...
            try:
                ...
                heatmap_sigma = getattr(args, "heatmap_sigma", 0.0)
                if heatmap_sigma > 0.0:
                    boundary_np = make_heatmap_labels(
                        one_hot_np,
                        sigma=heatmap_sigma,
                        mode=getattr(args, "heatmap_mode", "gaussian"),
                    )
                    # Dùng nhãn cứng để sampling (tránh đếm frame mờ là positive)
                    boundary_for_sampling = make_boundary_labels(one_hot_np, args.boundary_window)
                else:
                    boundary_np = make_boundary_labels(one_hot_np, args.boundary_window)
                    boundary_for_sampling = boundary_np
                idx = select_sample_indices(
                    one_hot_np,
                    boundary_for_sampling,
