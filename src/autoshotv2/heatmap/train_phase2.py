...
def _smooth_labels(targets: torch.Tensor, smoothing: float, asymmetric: bool) -> torch.Tensor:
    """Apply label smoothing to binary targets.

    asymmetric=True: only soften positives (1 → 1-ε), leave negatives at 0.
    asymmetric=False: symmetric (0 → ε/2, 1 → 1-ε/2).

    Asymmetric is preferred for shot boundary detection: temporal annotation
    uncertainty affects positive frames more than negative ones.
    """
    if smoothing <= 0.0:
        return targets
    if asymmetric:
        return targets * (1.0 - smoothing)
    return targets * (1.0 - smoothing) + (1.0 - targets) * (smoothing / 2.0)


class FocalLoss(nn.Module):
    def __init__(self, gamma: float = 1.0, alpha: float = 0.5, smoothing: float = 0.0, asymmetric: bool = True):
        super().__init__()
        self.gamma = gamma
        self.alpha = alpha
    ...
    def forward(self, logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        targets = _smooth_labels(targets, self.smoothing, self.asymmetric)
...
class LabelSmoothingBCE(nn.Module):
    """BCE with label smoothing for binary classification."""

    def __init__(self, smoothing: float = 0.1, asymmetric: bool = True):
        super().__init__()
        self.smoothing = smoothing
        self.asymmetric = asymmetric
...
class ManualAdam:
    ...
    def __init__(
        ...
        # GradScaler.unscale_() requires param_groups (list of dicts with 'params' key)
...
def train_head(
    ...
    loader = DataLoader(dataset, batch_size=args.batch_size, shuffle=True, num_workers=2 if use_amp else 0, pin_memory=use_amp)
    if args.loss == "focal":
        criterion = FocalLoss(gamma=args.gamma, alpha=args.alpha, smoothing=args.label_smoothing, asymmetric=args.ls_asymmetric)
    elif args.loss == "bce":
        criterion = LabelSmoothingBCE(smoothing=args.label_smoothing, asymmetric=args.ls_asymmetric) if args.label_smoothing > 0 else nn.BCEWithLogitsLoss()
    else:
    ...
    train_config = {
        ...
        "ls_asymmetric": args.ls_asymmetric,
    }
...
def main() -> None:
    ...
    # Heatmap boundary labels
    parser.add_argument("--heatmap-sigma", type=float, default=0.0,
                        help="Kích hoạt nhãn mềm dạng heatmap quanh boundary. "
                             "Giá trị = độ rộng (sigma) tính bằng frame (0=tắt, dùng hard label). "
                             "Gợi ý: 2.0-4.0. Kết hợp với --manyhot-weight để điều chỉnh trọng số.")
    parser.add_argument("--heatmap-mode", choices=["gaussian", "triangular", "cosine"], default="gaussian",
                        help="Hình dạng phân bố heatmap (default: gaussian).")
    # Label smoothing
    parser.add_argument("--label-smoothing", type=float, default=0.0,
                        help="Label smoothing epsilon (0=off). Recommended: 0.05-0.15 for shot boundary detection.")
    parser.add_argument("--ls-asymmetric", action="store_true", default=True,
                        help="Asymmetric smoothing: soften positives only (1→1-ε). More suitable than symmetric for boundary detection.")
    parser.add_argument("--ls-symmetric", dest="ls_asymmetric", action="store_false",
                        help="Symmetric smoothing: 0→ε/2, 1→1-ε/2.")
    # Kaggle T4 optimisations
    parser.add_argument("--amp", action="store_true",
                        help="Use automatic mixed precision (fp16). Recommended for Kaggle T4.")
