...
def render_tex_tables(manifest: dict[str, Any]) -> str:
    ...
    best_overview = comparisons["autoshotv2_best_sweep"]["metrics"]
    overview_rows = [
        "AutoShotV2 (cấu hình chính) & "
        rf"\textbf{{{f4(best_overview['shot'])}}} & "
        rf"\textbf{{{f4(best_overview['bbc'])}}} & "
        rf"\textbf{{{f4(best_overview['clipshots'])}}} & "
        "Ngưỡng tốt nhất theo từng bộ (giao thức AutoShot). \\\\",
    ...
    EVALUATION_PROCESS_LABELS = {
        ...
        "autoshotv2_best_sweep": "Kết quả chính (ngưỡng tốt nhất theo bộ)",
    }
    rows = []
    for identifier in comparison_order:
        ...
        rows.append(
            ...
            f"{tex_value(metrics['shot'], identifier == 'autoshotv2_best_sweep')} & "
            f"{tex_value(metrics['bbc'], identifier == 'autoshotv2_best_sweep')} & "
    ...
    shot = comparisons
    best_appendix = shot["autoshotv2_best_sweep"]["metrics"]
    lines += [
        ...
        rf"\textbf{{{f4(best_appendix['shot'])}}} \\",
        "}",
        "",
    ...
    # Deploy (fixed-threshold) experiments are excluded from the thesis: the
    # per-dataset best-threshold configuration is the single reported version.
    deploy_experiment_ids = {"phase2_deploy_threshold", "phase2_best_sweep"}
    rows = []
    for item in manifest["experiments"]:
        if item["id"] in deploy_experiment_ids:
            continue
        metrics = item["metrics"]
        rows.append(
